...
class NetworkClient(object):
	...
	def initialize(self):
		...
		if not os.path.exists(__location__+'/resources/source'): os.makedirs(__location__+'/resources/source') #stores common code
		...
		if query.startswith('n|'):
			...
			if not compat:
				if need != '':
					...
					if not confirm:
						return """Download aborted. Client requires script '%s' of version '%s' to connect to this server; client does not posses the script""" % (scriptname,scriptversion)
					print 'Download confirmed'
					protdownload = self.downloadProt(downloc)
					print protdownload
					if protdownload != '111':
						return """File not found on designated file server. Client requires script '%s' of version '%s' to connect to this server; client does not posses the script""" % (scriptname,scriptversion)
					self.gen_protlist()
					with open(__location__+'/resources/protocols/protlist.txt') as protlist:
						for line in protlist:
							if line == scriptname or line == scriptname + '\n':
								compat = True
					if not compat:
						return """Client requires script '%s' of version '%s' to connect to this server; client does not posses the script""" % (scriptname,scriptversion)
				else:
					return """Client requires script '%s' of version '%s' to connect to this server; client does not posses the script""" % (scriptname,scriptversion)
			...
			if preversion == scriptversion:
				...
				return """Client requires script '%s' of version '%s' to connect to this server; client has wrong version '%s'""" % (scriptname,scriptversion,preversion)
		else:
	...
	def downloadProt(self,downloc):
		...
		try:
			...
			return '404'
		return self.connectip(self,ip,[loc+'::/resources/protocols/'],'%s:%s:%s' % (scriptname,function,preversion))

	def confirmDownload(self,receivedip,need,downloc):
		confirm = raw_input('Server at %s points at %s to download the nessecary protocol %s. Proceed with download?\n>> ' % (receivedip,downloc.split('&&')[0],need))
		if confirm.lower() == 'y':
			return True
		return False
