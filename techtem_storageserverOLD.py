#!/usr/bin/python2
import sys, socket, select, os, threading, subprocess, random, sqlite3, getopt
...
class StorageServer(threading.Thread):
	...
	def init_spec(self):
		##################
		## Create an SQL database to keep track of all stored files, as well as a salt+hash for access to each of these files
		##################
		### server specific files start ###
		if not os.path.exists(__location__+'/resources/programparts/storage'): os.makedirs(__location__+'/resources/programparts/storage')
		if not os.path.exists(__location__+'/resources/programparts/storage/storagedatabase.sqlite3'):
			conn = sqlite3.connect(__location__+'/resources/programparts/storage/storagedatabase.sqlite3')
			cur = conn.cursor()
	...
	def sendCommand(self,s,AESkey):
		## Check credentials, perform command if appropriate
		pass

	def saveCommand(self,s,AESkey):
		## Check credentials, perform command if appropriate
		pass
	...
	def distinguishCommand(self,s,AESkey = None): # interpret what master requests
		...
		if order == 'send': # sync
			...
			self.sendCommand(s,AESkey)
		elif order == 'save':
			s.send('ok')
			print 'command understood, performing: %s' % order
			self.saveCommand(s,AESkey)
	...
	def servergen(self):
		global version
		print 'storage server started - version %s on port %s\n' % (version,self.serverport)
		self.get_netPass()
		netPass = self.netPass
				...
				if sock == serversocket:
					...
					try:
						...
						if rightPass == True:
							...
							if scriptname == 'storage' and function == 'storage_function' and cli_version == version:
								compat = 'y'
							...
							if compat != 'y': #not a sync_client, so respond with 
								...
								self.sendTem(s,'n|storage:storage_function:%s|' % version,AESkey)
...
def main(argv):
	...
	if portS == None:
		...
		try:
			...
			program = StorageServer(portI).start()
