...
class TemplateServer(CommonCode_Server.TemplateServer):
    ...
    def init_spec(self):
        ...
        if not os.path.exists(__location__ + '/resources/programparts/sync/syncdatabase.sqlite3'):
            ...
            cur = conn.cursor()
            cur.execute(
                'CREATE TABLE Accounts (salt TEXT, username TEXT, password TEXT, sizelim INTEGER, isAvailable INTEGER, locations TEXT, synctimestamp INTEGER)')
            conn.close()
        ...
        if not os.path.exists(__location__ + '/resources/programparts/%s/tokenservers.txt' % self.varDict["name"]):
            with open(__location__ + '/resources/programparts/%s/tokenservers.txt' % self.varDict["name"],
                      'wb') as tokenservs:
    ...
    def syncReceive(self, s, username):
        ...
        clientfiles = []
        with open(__location__ + '/resources/programparts/sync/%s/timestampclient.txt' % username,
                  'rb') as clienttimedoc:
        ...
        if not proper:
            ...
            if self.requireToken:
                ...
                if tokenServerIP == None:
                    ...
                    return
                validToken = tokenClient.connectip(tokenServerIP, requested_token + '|' + self.varDict["name"],
                                                   'checkout')
                if validToken:
                    s.sendall('y')
                else:
            ...
            cur.execute(
                'INSERT INTO Accounts (salt, username, password, sizelim, isAvailable, locations, synctimestamp) VALUES (?,?,?,?,?,?,?)',
                (salt, username, passwordHash, accountSizeLim, 1, synclocations, newtimestamp))
            conn.commit()
