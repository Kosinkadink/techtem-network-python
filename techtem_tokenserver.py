...
class TemplateServer(CommonCode_Server.TemplateServer):
    ...
    def init_spec(self):
        ...
        if not os.path.exists(__location__ + '/resources/programparts/%s' % self.name): os.makedirs(
            __location__ + '/resources/programparts/%s' % self.name)
        if not os.path.exists(__location__ + '/resources/programparts/token/tokendatabase.sqlite3'):
            ...
            cur = conn.cursor()
            cur.execute(
                'CREATE TABLE Tokens (token TEXT, server TEXT, creation INTEGER, expiration INTEGER, extra STRING)')
            conn.close()
    ...
    def createAndInsertToken(self, server, duration):
        ...
        try:
            ...
            # cur.execute('CREATE TABLE Tokens (token TEXT, server TEXT, creation INTEGER, expiration INTEGER,
            # extra STRING)')
            ...
            extra = ''
            cur.execute('INSERT INTO Tokens (token, server, creation, expiration, extra) VALUES (?,?,?,?,?)',
                        (tokenstring, server, creation, expiration, extra))
            conn.commit()
        except Exception, e:
            print str(e)
            tokenstring = None
